/*
 * TiMiniBrowser - 1.0
 * Titanium Appcelerator MiniBrowser
 * 
 * (c) 2011 - 2013 Rafael Kellermann Streit ( @rafaelks )
 * TiMiniBrowser may be freely distributed under the MIT license.
 *
 * This is a plugin that helpers you to create a internal browser
 * in your application.
 *
 * Example:
 ...
 * - barColor: You can set an hexadecimal color to MiniBrowser barColor.
 *       This color will be applied to NavigationBar, Toolbar
 *       and EmailDialog. Default is *undefined*.
 * - modal: Here, you set if the Window will be modal or not. If it's 
 *       not modal, the leftNavButton of Window will be *undefined*, and 
 *       you can open it using method *getWindow* and opening with a 
 *       NavigationGroup or a TabGroup. Default is *true*.
 *
 *
 * Methods:
 * - open: In iOS, this method will simple open the
 *       Window (probably Window have to be modal). In Android, will open an Intent.
 * - getWindow: Return the MiniBrowser Window instance, to 
 *       open with others components (like NavigationGroup and TabGroup).
 *
 */
...
TiMiniBrowser.prototype.createComponents = function() {
	var self = this;
	var hasToolbar = (typeof this.dict.showToolbar === "undefined") ? true : this.dict.showToolbar;
	...
	if (typeof this.dict.ga.trackScreen === "string") {
		var GA = require("/lib/Util/GoogleAnalytics");
		GA.trackScreen(this.dict.ga.trackScreen);
	}
